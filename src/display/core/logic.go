...
type LocationProcessingData struct {
	AgentCounter int
	ChunkCounter tui.Counter
	DbCounter    tui.Counter
}

// DataProcessingData holds counters for data processing phase (steps 4-18)
type DataProcessingData struct {
	CoordinateCount int
	BinCounter      tui.Counter
	AgentBinCount   int
	ScaleCounter    tui.Counter
	OutputScale     int
	OutputCounter   tui.Counter
	CleanupFiles    int
	CleanupDirs     int
	CleanupBytes    int
}
...
func NewBusinessLogic(cfg *ModuleConfig) *BusinessLogic { // Changed to config.ModuleConfig and renamed 'config' to 'cfg'
...
func (bl *BusinessLogic) SetChunkCounter(current, total int) {
	bl.locationProcessing.ChunkCounter = tui.Counter{Current: current, Total: total}
}

func (bl *BusinessLogic) SetDbCounter(current, total int) {
	bl.locationProcessing.DbCounter = tui.Counter{Current: current, Total: total}
}
...
func (bl *BusinessLogic) SetBinCounter(current, total int) {
	bl.dataProcessing.BinCounter = tui.Counter{Current: current, Total: total}
}
...
func (bl *BusinessLogic) SetScaleCounter(current, total int) {
	bl.dataProcessing.ScaleCounter = tui.Counter{Current: current, Total: total}
}
...
func (bl *BusinessLogic) SetOutputCounter(current, total int) {
	bl.dataProcessing.OutputCounter = tui.Counter{Current: current, Total: total}
}
