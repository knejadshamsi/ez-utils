...
func NewPTDisplayConfig(globalConfig *config.Config, cleanFlag bool) (*Config, error) {
	cfg, err := CreateConfigFromGlobal( // Renamed 'config' to 'cfg'
		globalConfig,
	...
	cfg.AddStep("Validate GTFS Files", "Checking required GTFS files", "file", "status", "size"). // Changed to cfg.AddStep
	...
	cfg.SetTheme("pt-blue"). // Changed to cfg.SetTheme
		SetFlag("clean", cleanFlag) // Changed to cfg.SetFlag
...
func NewPopulationDisplayConfig(globalConfig *config.Config, cleanFlag, dbFlag bool) (*Config, error) {
	...
	cfg.AddStep("Create Population Chunks", "Breaking population into manageable chunks", "count", "persons", "mb"). // Changed to cfg.AddStep
		...
		SetTheme("population-green"). // Changed to cfg.SetTheme
		SetFlag("clean", cleanFlag). // Changed to cfg.SetFlag
		SetFlag("db", dbFlag) // Changed to cfg.SetFlag
