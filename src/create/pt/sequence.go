// Step 7: Stop Sequence Collection
...
func (pto *PTOrchestrator) collectStopSequences() error {
	...
	// Read the trip mappings from Step 6
	...
	for i, header := range stopTimesHeaders {
		...
		stopTimesHeaderIndex[cleanHeader] = i
	}
	...
	for {
		...
		// Only process stops for trips we collected in Step 6
		if tripIDs[tripID] {
			...
			// Update live display every 500 sequences
			if sequenceCounter%500 == 0 && pto.tui != nil {
				pto.tui.UpdateCounter("sequences", sequenceCounter)
				pto.tui.UpdateCounter("stops", len(tripStops))
			}
