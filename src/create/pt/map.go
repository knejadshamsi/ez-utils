...
// This file is responsible for mapping service IDs to their corresponding routes.
// It reads service patterns and trips data to establish relationships between services and routes.
package pt
...
func (pto *PTOrchestrator) mapServiceRoutes() error {
	pto.logMessage("Starting Service-Route Mapping")

	// Initialize live updates for route mapping
	if pto.tuiEnabled && pto.tui != nil {
		pto.tui.UpdateCounter("routes", "0")
		pto.tui.UpdateCounter("mappings", "0")
	}
	...
	for i, header := range tripsHeaders {
		...
		tripsHeaderIndex[cleanHeader] = i
	}
	...
	for i, header := range routesHeaders {
		// Strip UTF-8 BOM from header if present (fixes STM GTFS data)
		cleanHeader := strings.TrimPrefix(header, "\uFEFF")
		routesHeaderIndex[cleanHeader] = i
	}
	...
	for serviceID, routes := range serviceToRoutes {
		...
		for routeID := range routes {
			...
			if pto.tuiEnabled && pto.tui != nil && pto.checkQuitRequested() {
			...
			if routeType, exists := routeTypes[routeID]; exists {
				...
				if batchUpdateCounter >= 10 || routeCounter == 1 {
					if pto.tuiEnabled && pto.tui != nil {
						pto.tui.UpdateCounter("routes", fmt.Sprintf("%d", routeCounter))
						pto.tui.UpdateCounter("mappings", fmt.Sprintf("%d", mappingCounter))
					}
					batchUpdateCounter = 0
				}
	...
	if pto.tuiEnabled && pto.tui != nil {
		pto.tui.UpdateCounter("routes", fmt.Sprintf("%d", routeCounter))
		pto.tui.UpdateCounter("mappings", fmt.Sprintf("%d service links", mappingCounter))
	}
