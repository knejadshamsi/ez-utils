...
import (
	...
	globalConfig "ez-utils/src/config"
)
...
type PTOrchestrator struct {
	...
	logFile       *os.File
	logger        *log.Logger
	displayInstance *display.DisplayInstance
...
func (pto *PTOrchestrator) initializeDisplay() error {
	...
	if pto.globalConfig != nil {
		config, err = display.NewPTDisplayConfig(pto.globalConfig, pto.config.CleanFlag)
	} else {
		// Fallback to default English
		config, err = display.NewConfig(
	...
	pto.displayInstance, err = display.NewDisplayInstance(config)
	if err != nil {
