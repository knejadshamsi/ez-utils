...
func (pto *PTOrchestrator) validateFiles() error {
	...
	for _, filename := range requiredFiles {
		...
		if pto.tuiEnabled && pto.tui != nil {
			pto.tui.UpdateCounter("file", filename)
			pto.tui.UpdateCounter("status", "validating")
		}
		...
		if fileInfo.Size() == 0 {
			...
			if pto.tuiEnabled && pto.tui != nil {
				pto.tui.UpdateCounter("status", "error")
			}
		...
		if pto.tuiEnabled && pto.tui != nil {
			pto.tui.UpdateCounter("size", fmt.Sprintf("%.1f KB", float64(fileInfo.Size())/1024))
			pto.tui.UpdateCounter("status", "valid")
		}
