// Step 9: Generate MATSim Transit Schedule XML
// This file is responsible for generating the final MATSim transit schedule XML output.
// It combines data from previous steps (trip mappings, stop details, stop sequences) to construct the XML.
package pt
...
func (pto *PTOrchestrator) generateXML() error {
	pto.logMessage("Starting MATSim Transit Schedule XML Generation")

	// Initialize live updates for XML generation
	if pto.tuiEnabled && pto.tui != nil {
		pto.tui.UpdateCounter("progress", "0%")
		pto.tui.UpdateCounter("size", "0 KB")
	}

	// 0. Read service mapping to get route types for transport mode mapping
	routeTypeMap := make(map[string]string)
	serviceMappingPath := filepath.Join(pto.config.TempDir, ServicesDir, ServiceMappingFile)
	if serviceMappingData, err := os.ReadFile(serviceMappingPath); err == nil {
		var serviceMappings []ServiceMapping
		if err := json.Unmarshal(serviceMappingData, &serviceMappings); err == nil {
			// Create route ID to route type lookup map
			for _, serviceMapping := range serviceMappings {
				for _, route := range serviceMapping.Routes {
					routeTypeMap[route.RouteID] = route.RouteType
				}
			}
			pto.logMessage(fmt.Sprintf("Loaded route type mappings for %d routes", len(routeTypeMap)))
		} else {
			pto.logMessage(fmt.Sprintf("Warning: Failed to parse service mapping JSON: %v", err))
		}
	} else {
		pto.logMessage(fmt.Sprintf("Warning: Could not read service mapping file: %v", err))
	}
...
`)
	...
	for _, details := range stopDetails {
		if !processedStops[details.StopID] {
			...
			if transitStopCounter%50 == 0 && pto.tuiEnabled && pto.tui != nil {
				pto.tui.UpdateCounter("progress", fmt.Sprintf("Stop %d", transitStopCounter))
			}
		}
	}
	...
	for routeID, tripIDs := range routeToTrips {
		...
		for tripIndex, tripID := range tripIDs {
			...
			if transportMode, hasMode := getTransportMode(routeTypeMap[routeID]); hasMode {
				xmlBuffer.WriteString(fmt.Sprintf("\t\t\t\t<transportMode>%s</transportMode>\n", transportMode))
			}
		...
		if routeCounter%10 == 0 && pto.tuiEnabled && pto.tui != nil {
			progress := int(float64(routeCounter) / float64(totalRoutes) * 100)
			pto.tui.UpdateCounter("progress", fmt.Sprintf("%d%%", progress))
			currentSize := float64(xmlBuffer.Len()) / 1024
			pto.tui.UpdateCounter("size", fmt.Sprintf("%.1f KB", currentSize))
		}
	...
	if pto.tuiEnabled && pto.tui != nil {
		...
		pto.tui.UpdateCounter("size", fmt.Sprintf("%.1f KB", finalSize))
	}
...
// getTransportMode maps GTFS route types to MATSim transport modes
func getTransportMode(routeType string) (string, bool) {
	switch routeType {
	case "1":
		return "metro", true
	case "3":
		return "bus", true
	default:
		return "", false // No match found - omit transportMode element
	}
}
