<script lang="ts">
  ...
  function startAddingNode() {
    networkState.isAddingNode = !networkState.isAddingNode;
    if (networkState.isAddingNode) {
      networkState.isAddingLink = false;
    }
  }
  
  function startAddingLink() {
    networkState.isAddingLink = !networkState.isAddingLink;
    if (networkState.isAddingLink) {
...
<div class="h-full flex flex-col bg-gray-50 dark:bg-gray-800 p-4">
  <!-- Part 1: Mode Toggle -->
  ...
  <div class="flex-shrink-0 mt-4 mb-4">
    {#if appState.networkMode === 'VIEW'}
      <Button 
        class="w-full" 
        color={networkState.isDrawingPolygon ? 'yellow' : 'primary'}
    ...
    {:else if appState.networkMode === 'CREATE'}
      <div class="space-y-2">
        <Button 
          class="w-full" 
          color={networkState.isAddingNode ? 'yellow' : 'primary'}
          onclick={startAddingNode}
        >
          <PlusOutline class="w-4 h-4 mr-2" />
          {networkState.isAddingNode ? 'Stop Adding Nodes' : 'Add Nodes'}
        </Button>
        <Button 
          class="w-full" 
          color={networkState.isAddingLink ? 'yellow' : 'alternative'}
          onclick={startAddingLink}
        >
          <PlusOutline class="w-4 h-4 mr-2" />
          {networkState.isAddingLink ? 'Stop Adding Links' : 'Add Links'}
        </Button>
      </div>
    {/if}
  </div>
