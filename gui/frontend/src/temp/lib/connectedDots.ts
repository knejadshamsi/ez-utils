...
function handleConnectedDrawingClick(event: L.LeafletMouseEvent) {
  ...
  const circleIcon = L.divIcon({
    html: `<div class="circle-marker" style="background-color: ${point.color}"><span>${mapState.connectedDots.showNumbers ? pointNumber : ''}</span></div>`,
    iconSize: [30, 30],
  ...
  marker.on('mouseover', () => {
    if (markerState === 'idle') {
      ...
      const hoverIcon = L.divIcon({
        html: `<div class="circle-marker" style="background-color: ${mapState.connectedDots.hoverPointColor}"><span>${mapState.connectedDots.showNumbers ? pointNumber : ''}</span></div>`,
        iconSize: [30, 30],
        className: 'custom-circle-icon'
      });
...
function handleConnectedEditMouseDown(_event: L.LeafletMouseEvent) {
  // Dragging is handled by Leaflet's built-in dragging
  // This is here for any additional edit mode logic
}

function setupConnectedInspection() {
...
// Helper to update number visibility
export function setShowNumbers(show: boolean) {
  ...
  mapState.connectedDots.points.forEach((point, index) => {
    const circleIcon = L.divIcon({
      html: `<div class="circle-marker" style="background-color: ${point.color || mapState.connectedDots.defaultPointColor}"><span>${show ? index + 1 : ''}</span></div>`,
      iconSize: [30, 30],
      className: 'custom-circle-icon'
    });
    point.marker.setIcon(circleIcon);
  });
}

// Set default colors for connected dots
export function setConnectedDotsColors(pointColor?: string, lineColor?: string, hoverPointColor?: string, hoverLineColor?: string) {
